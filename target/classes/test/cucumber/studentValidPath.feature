...
Feature: Use case of Student with valid paths
  ...
  @tag3
  Scenario Outline: A Student selects a course when the registraion period hasn't opened yet
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
  ...
  @tag6
  Scenario Outline: Two concurrent Students try to register the last place for a course when the registraion period opens
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create student <option1>
  ...
  @tag11
  Scenario Outline: A student registers a course for the last place and deregisters this course, anthoer student tries to register this course
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create student <option1>
    And This clerk enters student information, such as <student number1>, <name1> and <is fulltime(y/n)>
    ...
    Examples: 
      | option1          | student number1 | name1            | student number2 | name2          | is fulltime(y/n) | option2         | title                  | course code | capsize | enforce prereqs(y/n) | number of midterms | number of assignments | has a final(y/n) | is project course(y/n) | option3         | option4               | option5             |
      | "create student" |       101000001 | "John Masefield" |       101000002 | "Joe Thompson" | "y"              | "create course" | "Software Engineering" |      100000 |       1 | "n"                  |                  3 |                     5 | "y"              | "n"                    | "select course" | "register for course" | "deregister course" |
