...
Feature: Use Case of clerk with invalid paths
  ...
  @tag5
  Scenario Outline: A Clerk creates a course with an invalid input when the registration period hasn't opened yet
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create course <option>
    And This clerk enters course information, such as <title>, <course code>, <capsize>, <enforce prereqs(y/n)>, <number of midterms>, <number of assignments>, <has a final(y/n)> and <is project course(y/n)>
    ...
    Examples: 
      | option          | title              | course code | capsize | enforce prereqs(y/n) | number of midterms | number of assignments | has a final(y/n) | is project course(y/n) |
      | "create course" | "Game Development" |         100 |      30 | "n"                  |                 16 |                    19 | "y"              | "n"                    |
      | "create course" | "Game Development" |         102 |      30 | "n"                  |                 17 |                    19 | "y"              | "n"                    |
  ...
  @tag8
  Scenario Outline: A Clerk deletes a course when registration term ends
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create course <option1>
    And This clerk enters course information, such as <title>, <course code>, <capsize>, <enforce prereqs(y/n)>, <number of midterms>, <number of assignments>, <has a final(y/n)> and <is project course(y/n)>
  ...
  @tag11
  Scenario Outline: A Clerk creates a duplicate student when registration period hasn't opened yet
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create student <option>
    And This clerk enters student information, such as <student number>, <name> and <is fulltime(y/n)>
  ...
  @tag13
  Scenario Outline: A Clerk creates a student when the term ends
    Given The term ends
    And A clerk logs into the ATC successfully.
    When This clerk enters create student <option>
    And This clerk enters student information, such as <student number>, <name> and <is fulltime(y/n)>
  ...
  @tag14
  Scenario Outline: A Clerk creates a student with an invalid input when the registration period hasn't opened.
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create student <option>
    And This clerk enters student information, such as <student number>, <name> and <is fulltime(y/n)>
    ...
    Examples: 
      | option           | student number | name             | is fulltime(y/n) |
      | "create student" |         101001 | "John Masefield" | "y"              |
      | "create student" |         101002 | ""               | "y"              |
      | "create student" |         101003 | "Kerry Smith"    | "n"              |

  @tag15
  Scenario Outline: A clerk deletes a non-existent student when the registration period hasn't opened yet
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
  ...
  @tag18
  Scenario Outline: A clerk deletes a student when the term ends
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create student <option>
    And This clerk enters student information, such as <student number>, <name> and <is fulltime(y/n)>
  ...
  @tag20
  Scenario Outline: A clerk cancels a course when the registration hasn't opened yet
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create course <option1>
    And This clerk enters course information, such as <title>, <course code>, <capsize>, <enforce prereqs(y/n)>, <number of midterms>, <number of assignments>, <has a final(y/n)> and <is project course(y/n)>
  ...
  @tag22
  Scenario Outline: A clerk cancels a course when the term ends
    Given The registration period hasn't opened yet
    And A clerk logs into the ATC successfully.
    When This clerk enters create course <option1>
    And This clerk enters course information, such as <title>, <course code>, <capsize>, <enforce prereqs(y/n)>, <number of midterms>, <number of assignments>, <has a final(y/n)> and <is project course(y/n)>
